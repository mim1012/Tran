...
### 0.2 거래처 선택 탭 (2컬럼 리스트)

| 동작 | 설명 |
|------|------|
| **[+ 거래처 선택]** 클릭 | 새 탭 생성 → 거래처 리스트 (2컬럼) 표시 |
| **업체명 검색** | 실시간 필터링 |
| **거래처 카드 클릭** | 해당 거래처 상세화면으로 전환 |
| **탭 ✕ 클릭** | 해당 탭 닫기 |
...
| **최근거래 품목** | 우상단 | 자주 거래 품목, 수량 입력 → 발주/판매 작성 |
| **최근 거래 내역** | 하단 | 3개 탭: 등록, 최근작업(임시저장), 성사내역 |
